...
#
    ...
    for fileName in vel_files:
        ...
        for dataStep in new_dataSteps:
            ...
            #print(round(total_kinetic_energy_from_atoms/KE_0*100,1),'%')
            # percent2 = round(total_kinetic_energy_from_atoms/KE_0*100,1)
            # percentages.append(percent2)
            # percentages2.append(percent3)
            # starting_kinetic_energy = 6000 * 6000 *  0.5 * np.sum(mass)
            # percentage = round(total_kinetic_energy_from_atoms/starting_kinetic_energy*100, 1)
            # givenVelocityPercentages.append(percentage)
            # vel0 = np.ones((len(dataStep),3)) * 50000
            # KE0 = np.sum(mass/2*np.sum(vel0**2, axis=1))
            # percentage = (total_kinetic_energy_from_atoms/KE0*100)
            # givenVelocityPercentages.append(percentage)

        allPercentages.append(total_kinetics)

        # allPercentages.append(percent3)

        
        # with open((FILE.strip(".") + ".out"), "a") as out:
